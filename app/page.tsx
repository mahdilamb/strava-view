...
export default function Home() {
  ...
  const [targets, setTargets] = useState<ActivityTarget[]>([
    { name: "5km", predicate: activity => activity.distance >= 5_000, color: colorbrewer.Greens[3][2] },
    { name: "10km", predicate: activity => activity.distance >= 10_000, color: colorbrewer.Blues[3][2] },
    { name: "21.1km", predicate: activity => activity.distance >= 21_100, color: colorbrewer.Reds[3][2] }
  ])
  useEffect(() => {
  ...
  if (auth !== null) {
    return (
      <div className="flex flex-col h-dvh w-dvw">
        ...
        <Map targets={targets} activities={activities}></Map>
      </div>
    );
  }
  if (!stravaAuthUrl) {
